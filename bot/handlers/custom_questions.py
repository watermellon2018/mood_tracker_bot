...
вкл/выкл, архивирование. Сам ответ в ежедневном опросе — в bot/handlers/survey.py.
"""
...
# FSM states.
ASK_TEXT, ASK_TYPE, ASK_CONFIRM = range(3)
RENAME_AWAIT_TEXT = 0
...
async def _show_view(update: Update, tg_id: int, question_id: int) -> None:
    ...
    status = "Включен в ежедневный опрос" if q.is_enabled else "Выключен"
    text = (
        f"📝 Свой вопрос\n\n"
        f"Вопрос:\n«{q.question_text}»\n\n"
        f"Формат ответа:\n{TYPE_LABELS.get(q.answer_type, q.answer_type)}\n\n"
        f"Статус:\n{status}"
    )
    await _show(update, text, cq_view_keyboard(q))
...
async def cq_receive_type(
    ...
    text = context.user_data["cq_new"]["text"]
    await query.edit_message_text(
        f"Добавить этот вопрос в ежедневный опрос?\n\n"
        f"Вопрос:\n«{text}»\n\n"
        f"Формат ответа:\n{TYPE_LABELS[answer_type]}",
        reply_markup=cq_confirm_keyboard(),
    )
    return ASK_CONFIRM
...
async def cq_confirm(
    ...
    try:
        with session_scope() as session:
            ...
            q = custom_question_service.create(session, user.id, text, answer_type)
            qid = q.id
    ...
    return ConversationHandler(
        ...
        states={
            ...
            ASK_CONFIRM: [
                CallbackQueryHandler(
                    cq_confirm,
                    pattern=r"^cq:(confirm|edit_text|edit_type|cancel)$",
                ),
            ],
        },
